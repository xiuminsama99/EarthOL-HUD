...
只带飞不执行：接需求 → 选招 → 派发 Sub-Agent → 收反馈决策 → 派员体验 → 汇报人类。
...
派活前先做思维实验：把新功能与已有功能串起来，从打开到完成的完整使用流程每一步操作完整说一遍，确认新旧功能前后操作逻辑衔接合理顺手、提前消除隐性 bug，才可派活。
...
- #### 步骤四：派员体验（QA 由 Sub-Agent 完成）

  凡是能由 Sub-Agent 替代的人工环节（测试、检查、体验、反馈、审阅等），一律派 Sub-Agent 完成；能由 Sub-Agent 团队解决的，团队解决；其余自行推理解决。

  - **体验官视角**：派一个有专业背景的 Sub-Agent 完整走查，产出反馈报告（卡点、困惑点、改进建议）。
  - **纯小白视角**：再派一个零预设的 Sub-Agent，不给背景与设计文档，只以普通用户的身份直接体验，挑表达层问题（术语、文案、流程矛盾）。
  - **反馈闭环**：主 Agent 收反馈后自行决策修复，派 Sub-Agent 实现；修完派 Sub-Agent 复查，确认修复落地、无新增问题，循环到闭环。

  完成标准：体验反馈已处理、修复已复查通过。
  ...
  人类只拍板方向，测试、体验、检查由步骤四完成。涉及方向类问题，向人类推荐解决方案，其他问题自行推理解决：
